...
def RandomMix(img, ori_img, mmix, num, slabel):
    B, N, C = img.shape
    select = int(slabel[num])
    if select == B:
        return img
    else:
        W = int(((N) / 2.0) ** 0.5)
        ...
        M_w = int(((M_n) / 2.0) ** 0.5)
        M_h = int((M_n) / M_w)
        mmix = mmix.reshape(B, 1, M_h, M_w)
        mmix = F.interpolate(mmix, [H, W], mode='nearest')
        mmix = mmix.reshape(B, N, 1)
        img[num,:,:] = ori_img[num,:,:] * mmix[select,:,:] + ori_img[select,:,:] * (1.0 - mmix[select,:,:])
        return img
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, mmix, slabel, camera_id, view_id):
        B = x.shape[0]
        x = self.patch_embed(x)
        if self.training:
            ori_x = x.clone()
            for i in range (B):
                x = RandomMix(x, ori_x, mmix, i, slabel)
    ...
    def forward(self, x, mmix, slabel, cam_label=None, view_label=None):
        x = self.forward_features(x, mmix, slabel, cam_label, view_label)
        return x
